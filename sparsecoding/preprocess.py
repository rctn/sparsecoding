...
def patch_images(images, n_divisions=16):
    ...
    for i in range(n_images):
        im = images[:, :, i]
        patches = [im[x * patch_size_x: (x + 1) * patch_size_x, y * patch_size_y: (y + 1) * patch_size_y]
                   for x in range(0, n_divisions) for y in range(0, n_divisions)]
        patch_data[i] = torch.stack(patches)
    return patch_data, n_images, n_divisions, patch_size_x, patch_size_y


def unpatch_image(image, n_divisions, patch_size_x):
    ...
    for row in range(n_divisions):
        for col in range(n_divisions):
            final_img[row*patch_size_x: (row+1)*patch_size_x,
                      col*patch_size_x: (col+1)*patch_size_x] = image[index]
            index = index + 1
    return final_img
...
#                 image_idx
#             ].reshape(patch_dims[0]*patch_dims[1])
